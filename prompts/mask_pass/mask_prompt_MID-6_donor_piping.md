...
Important: trace the **inner edge** of the piping where it meets the cushion fabric, then include the full pipe width outward from that inner edge. If the outer edge is unclear, assume a typical pipe width and keep it consistent along the seam. If a pipe-path placement map is provided, it shows a **guide** for where the piping should be placed on the Donor Image.
